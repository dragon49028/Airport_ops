...
public class ConflictDetectionService {
    ...
    public ConflictResult checkGateConflict(String gate, LocalDateTime start, LocalDateTime end, Long excludeId) {
        ...
        if (excludeId != null) {
            conflicts.removeIf(assignment -> assignment.getId().equals(excludeId));
        }
    ...
    public List<String> suggestAvailableGates(LocalDateTime start, LocalDateTime end) {
        ...
        List<String> takenGates = allInWindow.stream()
            .map(GateAssignment::getGateNumber)
            .distinct()
            .toList();

        return ALL_GATES.stream()
            .filter(gate -> !takenGates.contains(gate))
            .limit(5)
            .toList();
    }
    ...
    public ConflictResult checkRunwayConflict(String runway, LocalDateTime slotTime, int duration, Long excludeId) {
        ...
        if (excludeId != null) {
            conflicts.removeIf(slot -> slot.getId().equals(excludeId));
        }
    ...
    public List<String> suggestAvailableRunways(LocalDateTime slotTime, int duration) {
        return ALL_RUNWAYS.stream().filter(runway -> {
            LocalDateTime end = slotTime.plusMinutes(duration);
            return runwayRepo.findConflicts(runway, slotTime, end).isEmpty();
        }).limit(3).toList();
    }
